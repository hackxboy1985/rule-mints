...
public class DebugMessageHolder {
	...
	@Value("${urule.htmlLogRemainTimeMintues}")
	private int htmlLogTimeMintues = 3;
	private int mintuesSecond = 60000;
	...
	public String getDebugMessage(String key){
		...
		String info=null;
		if(msg==null){
			info="<h2 style='color:red'>Key为["+key+"]的调试信息已失效，不能查看，当前key值为:"+this.num+"。<br>调试消息有效期为"+htmlLogTimeMintues+"分钟，请在有效期内查看!</h2>";
		}else{
			info=msg.getMessage();
		}
		clean();
...
//			if(dif>180000){
			if(dif>htmlLogTimeMintues*mintuesSecond){
				list.add(key);
			}
		}
		for(String key:list){
